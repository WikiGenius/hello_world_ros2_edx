...
def declare_launch_arguments():
    return [
        ...
        DeclareLaunchArgument('workcell_parent_name', default_value='world_interface'),
        ...
        DeclareLaunchArgument('robot2_pedestal', default_value='robot2_pedestal_'),
...
def load_xacro_file(file_path, arguments, param_name):
    return Node(
        package='xacro',
        executable='xacro',
        arguments=[os.path.join(FindPackageShare('hrwros_support'), 'urdf', file_path)] + arguments,
        output='screen',
        parameters=[{'name': LaunchConfiguration(param_name) + 'description'}]
    )

def spawn_model(name, model, param, x=None, y=None, z=None):
    args = ['-entity', LaunchConfiguration(model), '-topic', LaunchConfiguration(param) + 'description']
    if x is not None:
        args.extend(['-x', str(x)])
    if y is not None:
        args.extend(['-y', str(y)])
    ...
    return Node(
        ...
        name=LaunchConfiguration(name) + 'spawner',
        arguments=args,
        output='screen'
    )
